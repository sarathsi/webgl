...
var loadTextFileAsync = function(url, callback) {
  // SARATH
  console.log("console loadTextFileAsync() loading: " + url);
  log("loadTextFileAsync() loading: " + url);
  ...
  try {
    ...
    request.onreadystatechange = function() {
      log("onreadystatechange readyState=" + request.readyState + ", url: " + url);
      if (request.readyState == 4) {
        ...
        log("onreadystatechange() -> callback()");
...
var getFileListAsync = function(url, callback) {
  ...
  var getFileListImpl = function(url, callback) {
    var files = [];
    console.log("getFileListImpl() url: " + url);
    if (url.substr(url.length - 4) == '.txt') {
      loadTextFileAsync(url, function() {
        console.log("getFileListAsync() 1 loadTextFileAsync callback .txt file");
        return function(success, text) {
          console.log("getFileListAsync() 2 loadTextFileAsync callback .txt file");
          if (!success) {
            callback(false, '');
            return;
          }
          ...
          function finish(success) {
            ...
            log("count: " + count);
            if (!count) {
		console.log("loadTextFileAsync callback !count -> callback");
              callback(!fail, files);
            }
          }
        }
      ...
      console.log("getFileListAsync() loadTextFileAsync callback NOT .txt file. files.push url: " + url + ", files.size: " + files.size);
      files.push(url);
      callback(true, files);
    }
  };
...
var loadShaderFromFileAsync = function(
  ...
  loadTextFileAsync(file, function(gl, type, opt_errorCallback, opt_logShaders, file, opt_skipCompileStatus){
     ...
     log("loadShaderFromFileAsync() loadTextFileAsync callback: file: " + file);
      return function(success, shaderSource) {
        if (success) {
          log("loadShaderFromFileAsync() success");
          var shader = loadShader(gl, shaderSource, type, opt_errorCallback,
              opt_logShaders, undefined, file, opt_skipCompileStatus);
          log("loadShaderFromFileAsync() -> callback()");
